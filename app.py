...
def upload_zip():
    ...
    # Thêm Dockerfile cho file zip vừa giải nénnén
    dockerfile_path = os.path.join(project_extract_path, 'Dockerfile')
    # Mẫu Dockerfile auto thêm vào file
    ...
    # Nén lại file zip vừa thêm Dockerfile
...
def trigger_jenkins_build(zip_filename):
    payload = {
    # Truyền tên file chứ không truyền path
    ...
    logger.info(f" Kích hoạt Jenkins với {zip_filename} - Status: {response.status_code}")
